...
def debug(v):
	s = v['LABEL'] + ':' + v['ADM1'] + '|' + v['ADM2'] + '|' + v['ADM3'] + '|' + v['FIPS']
	return s
...
with open(data, 'r') as csvfile:
	for v in csv.DictReader(csvfile, dialect='piper'):
		...
		#if (v['LABEL'].startswith('Jefferson') and v['ADM2'] == 'Louisiana'):
		#	print(debug(v) + ' ' + v['DATE'])

for key in sorted(hash.keys()): # should already be sorted, but good hygiene
	v = hash.get(key)
	label = v['LABEL']
...
hashp2 = {}
stats2c = np.zeros((100,3))
stats2c[:] = np.nan # fill all as NaN
stats2d = np.zeros((100,3))
stats2d[:] = np.nan # fill all as NaN
...
print('Max   : ' + str(np.nanmax(stats2c, axis = 0)))
print('Mean  : ' + str(np.nanmean(stats2c, axis = 0)))
print('Median: ' + str(np.nanmedian(stats2c, axis = 0)))
print('StdDev: ' + str(np.nanstd(stats2c, axis = 0)))

print('\nState Died Statistics:')
print('Min   : ' + str(np.nanmin(stats2d, axis = 0)))
print('Max   : ' + str(np.nanmax(stats2d, axis = 0)))
print('Mean  : ' + str(np.nanmean(stats2d, axis = 0)))
print('Median: ' + str(np.nanmedian(stats2d, axis = 0)))
print('StdDev: ' + str(np.nanstd(stats2d, axis = 0)))
